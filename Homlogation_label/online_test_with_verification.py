...
for index_i,i in enumerate(model_list):
    ...
    file_text=open(f'{i}_start_exception.txt','r')
    temp_list_se=[]
    for fl in file_text:
        temp_list_se.append(fl.strip().replace(" ",""))
    file_text=open(f'{i}_exception.txt','r')
    temp_list_e=[]
    for fl in file_text:
        temp_list_e.append(fl.strip().replace(" ",""))
    model_dic[i]=[date_line_list[index_i],temp_list,temp_list_se,temp_list_e]
#print(model_dic)
...
def get_text_result(image,model):
    ...
    for i in lable_text_list:
        ...
        if flag:
            ...
            if i in model_dic[model][3]:
                result_list[cl]=True
                if not bypass_char_flag:
                    bypass_char_flag=True
                exceptional_index_list.append(cl)
                exceptional_file_index_list.append(model_dic[model][3].index(i))
            elif i in model_dic[model][2]:
                result_list[cl]=True
                bypass_char_flag=True
                exceptional_index_list.append(cl)
                exceptional_file_index_list.append(model_dic[model][2].index(i))
                print("first_line_exceptional")
            #print(i,model_dic[model][1][cl])
            cl=cl+1
            if cl==l_ml:
                break
    if not flag:
        print("first_line_not_detected")
    print(result_list,exceptional_index_list,exceptional_file_index_list)
    if bypass_char_flag:
        with open(f"report_{model}_ok_with_exception.txt","a") as f:
            f.write(lable_text)
            for i,j in zip(exceptional_index_list,exceptional_file_index_list):
                f.write(f"\n ell{i} using efl{j} ")
            print("exceptional")        
    elif all(result_list):
        with open(f"report_{model}_ok.txt","a") as f:
            f.write(lable_text)
            
    else:
        with open(f"report_{model}_nok.txt","a") as f:
            f.write(lable_text)
        ...
        if snap7.util.get_bool(data,0,0):# or i=="1":
            ...
            if result:
                ...
                if result:
                    ...
                    snap7.util.set_bool(data,0,2,True)
                    print(f"{model}_lable_text_retry_Nok")
                    cv2.imwrite(f"/home/rane123/Desktop/Homlogation_label/Nok_pics/img_{round(time.time())}.jpg",img)
                    nok_c=nok_c+1   
